# Runs the CLI without a local Python install.
#
#   docker run --rm -v "$PWD":/repo ghcr.io/aryamanz29/mergeproof check --local
#   docker run --rm -v "$PWD":/repo ghcr.io/aryamanz29/mergeproof explain
#
...
LABEL org.opencontainers.image.source="https://github.com/Aryamanz29/mergeproof" \
      org.opencontainers.image.description="Evidence gates for pull requests" \
...
RUN pip install --no-cache-dir --quiet "$(ls /wheels/*.whl)" && rm -rf /wheels
