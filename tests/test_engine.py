...
rules:
  ...
  - id: bugfix-evidence
    when: { title: "^fix" }
    instructions: Reproduce on ai-eval first.
    require:
      - check: evidence.field
        name: tenant
        with: { key: tenant, equals: ai-eval }
...
def test_evaluate_verdicts_and_report():
    ...
    assert tmpl["tenant"] == "ai-eval" and tmpl["traces"][0]["after"].startswith("https://www.braintrust.dev")
...
def test_explain_and_agent_prompt_render():
    ...
    assert "bugfix-evidence" in prompt and "Never fabricate evidence" in prompt and "tenant: ai-eval" in prompt
